...
/// </summary>
    ...
    public async Task HandlePreCheckoutAsync(PreCheckoutQuery query)
    {
        ...
        {
            await bot.AnswerPreCheckoutQueryAsync(query.Id, ok: false, errorMessage: "Инвойс не найден или уже оплачен.");
            return;
        }

        await bot.AnswerPreCheckoutQueryAsync(query.Id, ok: true);
    }
